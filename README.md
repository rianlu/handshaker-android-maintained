...
- **解锁剪切板功能**：通过硬编码 Smartisan 设备标识，使 Mac/Windows 客户端的剪切板 Tab 在任意 Android 设备上正常显示，并为非锤子设备补充前台单次同步能力
- 保留原始结构，适合继续做后续兼容性维护
...
- **非锤子手机**
  桌面端剪切板 Tab 可以正常显示；电脑→手机方向已可正常写入系统剪切板；手机→电脑方向则改为“回到 HandShaker 主页面时，尝试自动同步当前一条剪切板内容”。
...
- 当前已确认可用的是**非锤子设备**路径，尤其是“桌面端显示剪切板页签”“电脑→手机写入剪切板”“手机复制后回到 HandShaker 主页面自动同步一条到电脑端”这几个场景
...
- **非锤子设备手机→电脑剪切板只有锤子私有接口缺失时无法同步**（当前版本在保留锤子私有逻辑的同时，为非锤子设备补充了“回到 HandShaker 主页面时自动同步当前一条剪切板内容”的兼容方案；PC→手机方向粘贴已可用）
